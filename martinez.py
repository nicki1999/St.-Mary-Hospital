...
df_jennifer = df[df['Surgeon'] == 'Martinez, Adriana']
print(df_jennifer)
...
class MyWindow(QMainWindow):
    def __init__(self):
        ...
        if  jennifer_total_hip_count == 0:
                total_hip_percentage =0
        else:
            total_hip_percentage = count/jennifer_total_hip_count
        ...
        data = [["Alice", "25", "New York"],
                ...
                [str(jennifer_total_shoulder_count), str(count), str("{:.2f}".format((total_shoulder_percentage)*100)),
                 ...
                 str(jennifer_total_hip_count),str(jennifer_hip_count),str("{:.2f}".format((total_hip_percentage)*100))]]
        for i, row in enumerate(data):
            for j, item in enumerate(row):
                table.setItem(i, j, QTableWidgetItem(item))
                # Add new cells on top
        header_cell1 = QTableWidgetItem("Martinez, Adriana")
